...
def create_connections_discharge(network=None, charging_mode=True, Tsto_in=None, Tsto_out=None, Tenv=None):
    ...
    if charging_mode:
        ...
        c4 = Connection(cp, 'out1', hx1, 'in1', label='4')
        c5 = Connection(hx1, 'out1', hx2, 'in1', label='5')
        c6 = Connection(hx2, 'out1', cd, 'in1', label='6')
        c7 = Connection(cd, 'out1', ihx, 'in1', label='7')
        ...
        ihx.set_attr(pr1=1, pr2=1, kA=100)
        hx1.set_attr(pr1=1, kA=1e4) # ttd_l=5) # uncomment for model with environment temp setting
        hx2.set_attr(pr1=1, pr2=1, kA=1e4) # ttd_u=5) 
        ev.set_attr(pr1=1, pr2=1, ttd_u=5) #ttd_l = out1-in2, ttd_u = in1 - out2
        cp.set_attr(P=4.3e6)
        cd.set_attr(pr1=1, pr2=1)
        va.set_attr(pr=0.2)
        # Storage connection
        c24.set_attr(x=0, T=Tsto_out, fluid={"water": 1}) # T=40 for model with environment temp setting
        # c23.set_attr(T=110)
        # c22.set_attr(T=90)
        c21.set_attr(T=Tsto_in, x=0)
        # main connection
        # c1.set_attr(T=5)
        c2.set_attr(m=10, x=1, fluid={'NH3': 1})
        # c4.set_attr(h=2000)
        # c3.set_attr(Td_bp=30) # comment for Model with evironment temp setting
        # c7.set_attr(T=-10)
        # c7.set_attr(x=0)
...
network, gen, ph, ev, sh, ef_orc, el_orc = create_connections_discharge(network=nw, charging_mode=False, Tsto_in=70, Tsto_out=185, Tenv=10)
